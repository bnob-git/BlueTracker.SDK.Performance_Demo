    ...
    public class OnboardSampleSourceAssignmentClient : ApiWrapper
    {
        ...
        public OnboardSampleSourceAssignment Post(int imoNumber, string sourceName, DateTime validFrom)
        {
            return PostEmpty<OnboardSampleSourceAssignment>(
                $"/api/v1/ships/{imoNumber}/onboardSampleSourceAssignments?sourceName={Uri.EscapeDataString(sourceName ?? string.Empty)}&validFrom={Uri.EscapeDataString($"{validFrom:yyyy-MM-ddTHH:mm}")}");
        }
    }
}
